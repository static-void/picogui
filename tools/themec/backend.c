/* $Id: backend.c 3978 2003-05-23 10:19:38Z micah $
 *
...
void backend(void) {
  ...
  strncpy(themehdr->magic,"PGth", 4);   /* It's all 0 anyway, so ignore the null */
  ...
  while (op) {  /* For every theme object ... */
	...
	pp->ldnode->link_from = (unsigned long*) &propp->data;
      ...
      if (pp->link_from) {
		...
		*pp->link_from = htonll( ((char*)propp) - ((char*)themeheap) );
      }
  ...
  while (ldr) {
    ...
    if (sizeof(void*) == 4) {
        ...
        if ( ( ((char*)themeheap_p) - ((char*)themeheap) ) & 7)
           themeheap_p += 8 - (( ((char*)themeheap_p) - ((char*)themeheap) ) & 7);
    }
	...
	*ldr->link_from = htonll( ((char*)themeheap_p) - ((char*)themeheap) );
  ...
  for (c=0;c<num_thobj;c++,thop++) {
    ...
    proparray = (struct pgtheme_prop *) (((char*)themeheap) + 
		...
		proparray = (struct pgtheme_prop *) (((char*)themeheap) + be64toh(thop->proplist));
	  }
